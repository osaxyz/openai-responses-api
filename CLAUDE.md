# プロジェクト概要
1. このプロジェクト名は`OpenAI Responses API Schema`である
2. 製作者は`Arata Ouchi (Original SIN Architecture)`である
3. 製作者のX（Twitter）アカウントのURLは`https://x.com/ouchiarata`である
4. 製作者のホームページURLは`https://osa.xyz`である
5. ページタイトルは`OpenAI Responses API Schema`、日本語の短い説明は`OpenAI互換のエンドポイントを作成しやすくするための仕様群`、長い説明は`OpenAI互換の「OpenAI Responses API」互換エンドポイントを作成しやすくするため、実際にVercel AI SDKで問い合わせ、そのレスポンスを対応として記録したプロジェクトです。OpenAIおよびVercel AI SDKが更新され次第、順次対応します。`である
6. ドメインはCloudflareで管理されており、Vercelにデプロイされる
